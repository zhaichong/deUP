  ...
  <div class="space-y-4 max-w-3xl mx-auto pb-20">
    ...
    <div class="print-card bg-white rounded-3xl border border-slate-100 p-6 shadow-sm space-y-6 relative overflow-hidden">
      ...
      <div class="border-b border-slate-100 pb-4 text-center relative space-y-1">
        ...
        <h3 class="text-xs font-bold text-teal-700 tracking-wider pt-0.5">
          电子内镜检查报告单
        </h3>
      ...
      <!-- 2. 患者基本信息与就诊凭证表 -->
      <div class="bg-slate-50/80 p-3.5 rounded-2xl border border-slate-100/80 grid grid-cols-2 sm:grid-cols-4 gap-y-2.5 gap-x-4 text-xs">
        <div>
        ...
        <div>
          ...
          <span class="text-slate-700 font-medium">{{ record.department || '消化内镜室' }}</span>
        </div>
        <div>
        ...
        <div>
          ...
          <span class="text-teal-700 font-bold">已审核发布</span>
        </div>

        <div v-if="record.idCard" class="col-span-2 sm:col-span-4 pt-1 border-t border-slate-200/50">
      ...
      <div class="pt-4 border-t border-slate-100 flex items-center justify-between text-[11px] text-slate-400">
        ...
        <span>报告出具日期: {{ record.examDate }}</span>
      </div>
    </div>
